...
public class BurpExtender extends AbstractTableModel implements IBurpExtender, ITab, IHttpListener,IScannerCheck, IMessageEditorController,IContextMenuFactory
{
    ...
    public void registerExtenderCallbacks(final IBurpExtenderCallbacks callbacks)
    {
        ...
        this.stdout.println("hello sqlinject_test!");
        ...
        callbacks.setExtensionName("sqlinject_test");
        ...
        SwingUtilities.invokeLater(new Runnable()
        {
            ...
            public void run()
            {
                ...
                JLabel jls=new JLabel("插件名:sqlinject_test");    //创建一个标签
                JLabel jls_1=new JLabel("版本： 1.0 ");    //创建一个标签
                ...
                JLabel jls_2=new JLabel("自己可以填写或修改payload");    //创建一个标签
                JCheckBox chkbox4=new JCheckBox("测试Cookie");    //创建指定文本的复选框
                JLabel jls_3=new JLabel("如果需要多个域名加白请用,隔开");    //创建一个标签
                JTextField textField = new JTextField("填写白名单域名");//白名单文本框


                JButton btn1=new JButton("清空列表");    //创建JButton对象
                JButton btn2=new JButton("重新加载payload");    //创建JButton对象
                JButton btn3=new JButton("启动白名单");    //处理白名单

                //自定义payload区
                JPanel jps_2=new JPanel();
                jps_2.setLayout(new GridLayout(1, 1)); 
                JTextArea jta=new JTextArea("' and sleep(3)%23",18,16);

                //读取ini配置文件
                try {
                    BufferedReader in = new BufferedReader(new FileReader("sqlinject_test.ini"));
                    String str,str_data="";
                    while ((str = in.readLine()) != null) {
                        str_data += str+"\n";
						JTextArea_data_1 += str+"\n";
                    }
                ...
                chkbox1.addItemListener(new ItemListener() {
                    ...
                    public void itemStateChanged(ItemEvent e) {
                        if(chkbox1.isSelected()){
                            stdout.println("插件sqlinject_test启动");
                            switchs = 1;
                        }else {
                            stdout.println("插件sqlinject_test关闭");
                            switchs = 0;
                        }
                ...
                btn2.addActionListener(new ActionListener() {//加载自定义payload
                    ...
                    public void actionPerformed(ActionEvent e) {
						...
						//写入ini配置文件
						try {
							BufferedWriter out = new BufferedWriter(new FileWriter("sqlinject_test.ini"));
    ...
    public List<JMenuItem> createMenuItems(final IContextMenuInvocation invocation) {
        ...
        if(invocation.getToolFlag() == IBurpExtenderCallbacks.TOOL_REPEATER || invocation.getToolFlag() == IBurpExtenderCallbacks.TOOL_PROXY){
            ...
            JMenuItem jMenu = new JMenuItem("Send to sqlinject");
            ...
            jMenu.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    if(switchs == 1) {
                        ...
                        BurpExtender.this.stdout.println("插件sqlinject关闭状态！");
                    }
